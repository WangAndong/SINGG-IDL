PRO plot_vel_multi, fcomb, hardfile=hardfile, jpg=jpg, $
                    titles=titles, background=background, noempty=noempty
   ffpos      =  'fiberpos.txt'
   ...
   pi         = 3.14159265
   cvel       = 299792.458
   nplots     = n_elements(fcomb)
   layout     = [2,ceil(nplots/2.0)]
   ;
   ...
   FOR jj = 0, nplots-1 DO BEGIN
       ;
       ; read data
       readcol, fcomb(jj), apid, z1, f1, z2, f2, z3, f3, z4, f4, z5, f5, z6, f6, $
       ...
       IF keyword_set(background) THEN BEGIN
           ...
           readcol, fbacklist, bgname, bgfname, bgra1, bgra2, bgdec1, bgdec2, $
             format='(a,a,a,a,a,a)'
           bg         = where(bgname EQ background(jj))
           ;
           ; load image
           ...
           combpos    = strpos(fcomb(jj), 'comb')
           ffits      = strmid(fcomb(jj), 0, combpos) + 'obj' + $
             strmid(fcomb(jj), combpos+4, 13) + '.co.cr.ms.fits'
       ...
       FOR ii = 0, n_elements(fid)-1 DO BEGIN
           ...
           IF keyword_set(noempty) AND isapinlist EQ 0 THEN plotflag = 0
