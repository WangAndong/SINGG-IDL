pro create_struct2, struct, strname, tagnames, tag_descript, DIMEN = dimen, $
...
;
;       Unlike the intrinsic function CREATE_STRUCT2(), this procedure does not
;       require the user to know the number of tags before run time.   (Note
;       there is no name conflict since the intrinsic CREATE_STRUCT2 is a 
;       function, and this file contains a procedure.)
; CALLING SEQUENCE:
;       CREATE_STRUCT2, STRUCT, strname, tagnames, tag_descript, 
...
;
;       CHATTER -  If /CHATTER is set, then CREATE_STRUCT2 will display
...
;                  CREATE_STRUCT2 will not be deleted upon exiting.   See below
;
; OUTPUTS:
...
; EXAMPLES: 
;
;       IDL> create_struct2, new, 'name',['tag1','tag2','tag3'], 'D(2),F,A(1)'
;
...
;
; NOTES:
;       If CREATE_STRUCT2 cannot write a temporary .pro file in the current 
...
;-------------------------------------------------------------------------------
 ...
 if N_params() LT 4 then begin
   print,'Syntax - CREATE_STRUCT2, STRUCT, strname, tagnames, tag_descript,' 
