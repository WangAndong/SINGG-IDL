pro mysky,image,skymode,skysig,CIRCLERAD=circlerad,MASK=mask,SILENT=silent
;+
; NAME:
;       SKY
; PURPOSE:
;       Determine the sky level in an image using the the procedure MMM2
; EXPLANATION:
...
;
; CALLING SEQUENCE:
;       SKY, image, [ skymode, skysig, CIRCLERAD = ,MASK = ,/SILENT ]
;
; INPUTS:
...
;               array IMAGE, as determined by the procedure MMM2.
...
;               of the image.
;       MASK    - Don't include any pixels where mask=1b
;
...
;       Added "mask" capability  D. Hanish      2005ish
;-
 On_error,2              ;Return to caller
 maxsky = 32768          ;Maximum # of pixels to be used in sky calculation
...
; mask is 1b on pixels we want to reject
       ngood=0
       good = INTARR(npts)
       FOR ii = 0,npts-1 DO BEGIN
         IF NOT mask[xx[ii],yy[ii]] THEN BEGIN
           good[ngood]=ii
           ngood = ngood + 1
         ENDIF
       ENDFOR
;       good = WHERE((NOT mask[xx,yy]),ngood)

;       PRINT,"ngood=",ngood," out of",N_ELEMENTS(index)
       IF ngood GT 0 THEN BEGIN
          index = index[good[0:ngood-1]] 
       ENDIF ELSE BEGIN
         PRINT,"ERROR - no pixels remaining after mask subtraction"
; Return dummy values
          skymode = -999.0
          skysig = -999.0
          RETURN
 ...
 mmm2, image[index], skymode, skysig
