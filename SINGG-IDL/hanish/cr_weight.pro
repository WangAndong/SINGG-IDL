PRO cr_weight, input_cube, rd_noise_dn, dark_dn, gain, mult_noise, $
...
;
; CALLING SEQUENCE:
;     cr_weight, input_cube, rd_noise_dn, dark_dn, gain, mult_noise, $
...
;                     1 - Noise cube inverse variance weighting.
;                         That is, the weight of each pixel is 1/(noise^2)
;                     2 or more - Sky and read noise weighting of rates.
...
;     weight_array - Uses a specified image-by-image weight instead of 
;                    the above methods.  May only be used if weighting=0,
...
;
;     Cr_weight emulates the crrej routine in stsdas.hst_calib.wfpc.
...
;     does.  Away from the edge, crrej and cr_weight are the same with
;     respect to dilation.
;
;     The main difference between crrej and cr_weight is in the sky
;     computation.  cr_weight does a DAOPHOT I sky computation on a 
...
;      4 Apr. 1997 - Changed name to cr_weight.  RSH
...
IF n_params(0) LT 6 THEN BEGIN
    ...
    print,'   input_mask, weighting, skybox, weight_array'
    return
ENDIF
...
IF sz[0] NE 3 THEN BEGIN
    print,'CR_WEIGHT:  Input cube must have 3 dimensions.'
    return
ENDIF
...
IF n_elements(input_mask) GT 0 THEN BEGIN
    ...
    IF cwsz GT 0 THEN BEGIN
        print,'CR_WEIGHT:  INPUT_MASK must be same size as IMAGE_CUBE.'
        return
    ENDIF ELSE BEGIN
        IF verbose THEN print,'CR_WEIGHT:  Using INPUT_MASK.'
    ENDELSE
    use_input_mask = 1b
ENDIF ELSE BEGIN
    use_input_mask = 0b
...
IF (usemean + usemedian + usemin) GT 1  THEN BEGIN
    print,'CR_WEIGHT:  Specify only one of MEDIAN_LOOP, MEAN_LOOP' $
      + ', or MINIMUM_LOOP'
    return
ENDIF
...
IF (inimean + inimed + inimin) GT 1  THEN BEGIN
    print,'CR_WEIGHT:  Specify only one of INIT_MED, INIT_MEAN,' $
      + ' or INIT_MIN.'
    return
ENDIF
...
IF nimg LT 3 AND inimed THEN BEGIN
    ...
    IF verbose THEN BEGIN
        print,'CR_WEIGHT:  INIT_MED only permitted for 3 or more ' $
            + 'images.'
...
IF keyword_set(noclearmask) THEN BEGIN
    clearmask = 0b
    IF verbose THEN print,'CR_WEIGHT:  CR flags accumulate strictly.'
ENDIF ELSE BEGIN
    clearmask = 1b
    IF verbose THEN print,'CR_WEIGHT:  CR flags cleared between iterations.'
ENDELSE 
;
;  Default iterations.
;
...
IF verbose THEN BEGIN
    print,'CR_WEIGHT: Iteration spec:  '
    print,'           nsig = ',nsig
...
IF n_elements(exptime) NE 0 THEN BEGIN
    IF n_elements(exptime) NE nimg THEN BEGIN
        print,'CR_WEIGHT:  EXPTIME must have one element per input image.'
        return
    ENDIF
    zexp = 0b
    ...
    IF zexp THEN BEGIN
        ...
        IF verbose THEN print, $
          'CR_WEIGHT:  All exposure times <= 0.'
    ENDIF
ENDIF ELSE BEGIN
    zexp = 1b
...
IF n_elements(weighting) GT 0 THEN BEGIN
    wgt = weighting
    wgt = round(wgt)
    IF wgt ne 0 and wgt ne 1 and wgt ne 2 THEN BEGIN
        print, 'CR_WEIGHT:  Weighting must be 0, 1, or 2'
...
IF verbose THEN BEGIN
    ...
    CASE wgt OF
        ...
        1:  print,'           weighting of rate by input noise cube'
        2:  print,'           weighting of rate by sky and read noise'
    ENDCASE
ENDIF
...
IF keyword_set(dilation) OR keyword_set(dfactor) THEN BEGIN
    ...
    IF (dilation LE 0) OR (dfactor LE 0) THEN BEGIN
        print,'CR_WEIGHT:  Dilation specs not valid: '
    ...
    IF verbose THEN BEGIN
        print,'CR_WEIGHT:  Dilation will be done.  Specs:'
        print,'           dilation = ',dilation
        print,'           dfactor  = ',dfactor
        print,'           kernel = '
        print,kernel
...
ENDIF ELSE BEGIN
    do_dilation = 0b
    IF verbose THEN print,'CR_WEIGHT:  Mask dilation will not be done.' 
ENDELSE


IF verbose THEN print,'CR_WEIGHT:  Initializing noise and mask cube.'
IF rd_noise_dn GE 0 THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Noise cube computed.'
    supplied = 0b
    ...
    ENDFOR
ENDIF ELSE BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Noise cube supplied.'
    supplied = 1b
    IF wgt EQ 2 THEN BEGIN
        print, 'CR_WEIGHT:  WEIGHTING=2 incompatible with supplying ', $
            'noise cube.'
        print, '            Executing return.'
        return
    ENDIF
...
IF keyword_set(noskyadjust) THEN BEGIN
    ...
    totsky = 0
ENDIF ELSE BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Sky adjustment being made.'
    skyadj_cube, input_cube, skyvals, totsky, $
      verbose=verbose, xmedsky=xmed, input_mask=input_mask, $
      region=skybox
ENDELSE

IF verbose THEN print,'CR_WEIGHT:  Scaling by exposure time.'
...
IF inimed THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Initializing with median.'
...
ENDIF ELSE IF inimean THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Initializing with mean.'
    IF use_input_mask THEN BEGIN
...
ENDIF ELSE IF inimin THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Initializing with minimum.'
    IF use_input_mask THEN BEGIN
    ...
    ENDELSE
ENDIF ELSE BEGIN
    print,'CR_WEIGHT:  Logic error in program initializing check image.'
    return
ENDELSE
;
...
IF track THEN BEGIN
    print,'CR_WEIGHT:  Tracking.  Iter = ',strtrim(iter,2)
...
ENDIF
IF verbose THEN BEGIN
    print,'CR_WEIGHT:  Beginning iteration number ',strtrim(iter,2)
...
;
    ...
    IF count GT 0 THEN BEGIN
        mask_cube[i*npix + cr_subs[0:count-1]] $
          = replicate(0b,count)
    ENDIF
    IF verbose THEN print,'CR_WEIGHT:  ',strtrim(count,2), $
...
;
    ...
    IF do_dilation THEN BEGIN
        ...
        IF dct GT 0 THEN BEGIN
            ...
            ENDIF
        ENDIF
        IF verbose THEN print,'CR_WEIGHT:  Mask dilation performed.  ', $
...
IF usemedian THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Taking median.'
    IF use_input_mask THEN BEGIN
...
ENDIF ELSE IF usemean THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Taking mean.'
    IF use_input_mask THEN BEGIN
...
ENDIF ELSE IF usemin THEN BEGIN
    IF verbose THEN print,'CR_WEIGHT:  Taking minimum.'
    IF use_input_mask THEN BEGIN
    ...
    ENDELSE
ENDIF ELSE BEGIN
    print,'CR_WEIGHT:  Logic error in program recomputing check image.'
    return
ENDELSE
...
IF verbose THEN BEGIN
    FOR i=0,nimg-1 DO BEGIN
      ...
      ncut_tot[i] = count
    ENDFOR
    print,'CR_WEIGHT:  Total pixels changed:  '
    print,ncut_tot
ENDIF

IF track THEN BEGIN
    print,'CR_WEIGHT:  Tracking.  After loop exit.'
    print,'   Combined_image:  '
    print,combined_image[tracking_set]
;    print,'   Current_var:  '
;    print,current_var[tracking_set]
...
IF wgt EQ 0 THEN BEGIN
  wgts = xrepl # exptime
  IF KEYWORD_SET(weight_array) THEN BEGIN
    wgts = MAKE_ARRAY(xdim,nimg,value=1.0)
    FOR ii=0,nimg-1 DO BEGIN
      wgts[*,ii] = weight_array[ii]
    ENDFOR
  ENDIF
ENDIF ELSE BEGIN
...
columnnoise = FLTARR(ydim)
FOR i=0,nimg-1 DO BEGIN
  plane_wgts = wgts[*,i] # yrepl
  IF wgt EQ 1 THEN BEGIN
; First, calculate "average noise" for an image to normalize
    tempweight = noise_cube[*,*,i]^(-2.0)
    input_cube[0,0,i] = input_cube[*,*,i] / noise_cube[*,*,i]^2.0
    noise_cube[0,0,i] = 1.0 / noise_cube[*,*,i]
    FOR jj = 0, xdim-1 DO BEGIN
; w = 1/n^2 = 1/(scale^2 * (img + rd^2)), and we want w=1/scale, so multiply by
; scale*img, which is the input cube mean?
      plane_wgts[jj] = MEAN(noise_cube[jj,*,i]^(-2.0),/NAN)
    ENDFOR
  ENDIF ELSE BEGIN
...
IF verbose AND c0 GT 0 THEN $
      print,'CR_WEIGHT:  ',strtrim(c0,2),' pixels rejected on all inputs.'
...
IF keyword_set(bias) THEN BEGIN
    print,'CR_WEIGHT:  Bias flag set -- returning mean instead of total.'
...
IF keyword_set(restore_sky) THEN BEGIN
    IF wgt EQ 0 THEN BEGIN
        IF verbose THEN print,'CR_WEIGHT:  Adding sky back into data cube'
        IF xmed THEN BEGIN
            FOR i=0,nimg-1 DO BEGIN
        ...
        ENDELSE
    ENDIF ELSE BEGIN
        print, 'CR_WEIGHT:  /RESTORE_SKY ignored because weighting spec ' $
            + 'not zero.'
    ENDELSE
ENDIF
