PRO build_header_db,RUNLIST=runlist,VERBOSE=verbose
  ...
  FOR irun = 0,n_runs-1 DO BEGIN
    ...
    FOR ii = 0,n_objects-1 DO BEGIN
      ...
      IF NOT FILE_TEST(imlist[0]) THEN BEGIN
        PRINT,'WARNING in build_header_db: directory contains no sky-subtracted images ',object
        n_files = 0
      ENDIF
...
; below SHOULD cover it, but it's one we really need done right.
        ...
        IF count NE 1 THEN BEGIN
          PRINT,'ERROR in build_header_db: IMTYPE must be set ',imlist[jj]
          RETURN
        ENDIF

;        IF STRPOS(imlist[jj],'sub_ss.fits') GT 0 THEN BEGIN
;          header.imtype="net"
;        ENDIF ELSE BEGIN
;          IF STRPOS(imlist[jj],'_6') GT 0 THEN BEGIN
;            header.imtype="onband"
;          ENDIF ELSE BEGIN
;            IF STRPOS(imlist[jj],'_R_') GT 0 OR STRPOS(imlist[jj],'_C_') GT 0 THEN BEGIN
;              header.imtype="cont"
;            ENDIF ELSE BEGIN
