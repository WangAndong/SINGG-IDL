pro readcol_new,name,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13,v14,v15, $
            v16,v17,v18,v19,v20,v21,v22,v23,v24,v25, COMMENT = comment, $
            FORMAT = fmt, DEBUG=debug, SILENT=silent, SKIPLINE = skipline, $
            NUMLINE = numline, DELIMITER = delimiter, QUOTES = quotes
;+
; NAME:
;       READCOL_NEW
; PURPOSE:
...
;
; CALLING SEQUENCE:
;       READCOL_NEW, name, v1, [ v2, v3, v4, v5, ...  v25 , COMMENT=
...
;       /QUOTES - Treats any text included between either single (')
;                 or double (") quotes to be one string entry, not to
;                 be divided up.
;
; OUTPUTS:
...
;       IDL> READCOL_NEW,'position.dat',F=FMT,name,hr,min,sec,deg,dmin,dsec  
;
...
;       floating point.
;
;       IDL> READCOL_NEW,'position.dat',F='A',name,hr,min,sec,deg,dmin,dsec 
;
;       To read just the variables HR,MIN,SEC
;       IDL> READCOL_NEW,'position.dat',F='X,I,I,F',HR,MIN,SEC
;
; RESTRICTIONS:
...
;-
  ...
  if N_params() lt 2 then begin
     print,'Syntax - readcol_new, name, v1, [ v2, v3,...v25, '
   ...
   for j = 0L, nlines-1 do begin
    ...
    for i = 0L,nfmt-1 do begin
       ...
       IF KEYWORD_SET(quotes) THEN BEGIN
         temp = STRTRIM(temp,1)
         length = STRLEN(temp)
         singlepos = STRPOS(temp,"'")
         doublepos = STRPOS(temp,'"')
         nonepos = STRPOS(temp,delimiter)
         IF (singlepos LT 0 OR nonepos LT singlepos) AND $
            (doublepos LT 0 OR nonepos LT doublepos) THEN BEGIN
           var = gettok(temp,delimiter)
         ENDIF ELSE BEGIN
           IF (singlepos GT 0 AND (doublepos LT 0 OR singlepos LT doublepos)) THEN BEGIN
             startpos = singlepos
             tempstr = STRMID(temp,startpos+1,length-startpos-1)
           ...
           ENDIF ELSE BEGIN
             ...
             endpos = STRPOS(tempstr,'"')
           ENDELSE
           var = STRMID(temp,startpos+1,endpos-startpos)
         ENDELSE
       ENDIF ELSE BEGIN
