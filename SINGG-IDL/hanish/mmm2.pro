pro mmm2, sky_vector, skymde, sigma , skew, HIGHBAD = highbad, DEBUG = debug
;+
; NAME:
;       MMM2
; Changes from MMM:
;  1> mxiter was changed, from 30 to 100
;  2> if we run out of iterations it doesn't reset sigma to -1.
;  3> "delta" is now a DOUBLE, and DOUBLE(istep) is used in sum and sumsq.
;     This keeps the algorithm from crashing if images with cosmic rays are
;     used as input.

; PURPOSE: 
;       Estimate the sky background in a stellar contaminated field.
; EXPLANATION:  
;       MMM2 assumes that contaminated sky pixel values overwhelmingly display 
;       POSITIVE departures from the true value.  Adapted from DAOPHOT 
;       routine of the same name.
;
; CALLING SEQUENCE:
;       MMM2, sky, [ skymde, sigma, skew, HIGHBAD = , /DEBUG  ]
;
; INPUTS:
;       SKY - Array or Vector containing sky values.  This version of
;               MMM2 does not require SKY to be sorted beforehand.  SKY
...
;
; METHOD:
;       The algorithm used by MMM2 consists of roughly two parts:
...
;-
 ...
 mxiter = 100             ;Maximum number of iterations allowed
 minsky = 20              ;Minimum number of legal sky elements
 if N_params() EQ 0 then begin          
        print,"Syntax:  MMM2, sky, skymde, sigma, skew, [HIGHBAD = , /DEBUG] "
        return
 endif

 nsky = N_elements( sky_vector )            ;Get number of sky elements     
 if nsky LE minsky then message, $   
    'ERROR -Input vector  contains '+strtrim(nsky,2)+' elements, must contain at least '+strtrim(minsky,2)+' elements'
...
; Select the pixels between Cut1 and Cut2
 ...
 delta = DOUBLE(sky[good] - skymid)  ;Subtract median to improve arithmetic accuracy
 sum = total(delta)                     
 sumsq = total(delta^2)
...
; Change from MMM: if we run out of iterations, don't reset sigma and don't
...
;
    ...
    if not done then begin
        ...
        endrep until done
        if tst_min then delta = DOUBLE(sky[newmin+1:minmum] - skymid) $
                   else delta = DOUBLE(sky[minmum+1:newmin] - skymid)
        sum = sum - DOUBLE(istep)*total(delta)
        sumsq = sumsq - DOUBLE(istep)*total(delta^2)
        redo = 1b
        minmum = newmin
     endif
;       
   ...
   if not done then $   
    ...
    if not done then begin                 ;Keep incrementing NEWMAX
       ...
       endrep until done
       if tst_max then delta = DOUBLE(sky[maxmum+1:newmax] - skymid) $
               else delta = DOUBLE(sky[newmax+1:maxmum] - skymid)
       sum = sum + DOUBLE(istep)*total(delta)
       sumsq = sumsq + DOUBLE(istep)*total(delta^2)
       redo = 1b
       maxmum = newmax
...
;
;;   sum = TOTAL(sky[minmum+1:maxmum]-skymid)
;;   sumsq = TOTAL((sky[minmum+1:maxmum]-skymid)^2.0)
;; if these are used (and they give the same results), remove the sum, sumsq,
;; and "if tst_m?? then delta" lines from above.
...
;       
 ...
 if keyword_set(DEBUG) or ( N_params() EQ 1 ) then begin
        print, '% MMM2: Number of unrejected sky elements: ', strtrim(nsky,2), $
              '    Number of iterations: ', strtrim(niter,2)
        print, '% MMM2: Mode, Sigma, Skew of sky vector:', skymde, sigma, skew   
 endif
