PRO pfplt_extractprof, file, netflag, sma, fint, dfint, ngood, nbad, sb, esb, $
   ...
   ;
   ; G. Meurer 10/2004
   ; D. Hanish 09/2005 calls to readcol made silent; fixed parea bug in errors
   ;
   ...
   IF netflag THEN $
    readcol, file, sma, fint, dfint, ngood, nbad, sb, esb, dfraw, $
             efintsk, efintcn, format=fmtn, skipline=skipline, numline=numline,/silent ELSE BEGIN 
      readcol, file, sma, fint, dfint, ngood, nbad, sb, esb, dfraw, $
               efintsk, format=fmtc, skipline=skipline, numline=numline,/silent
      efintcn = 0.0 * efintsk
   ENDELSE 
   kk     = where(sma GT 0.0)
   ;
   ...
   dfraw  = dfraw[kk] * fsc
   efintsk  = efintsk[kk] * fsc
   efintcn  = efintcn[kk] * fsc
   ;
