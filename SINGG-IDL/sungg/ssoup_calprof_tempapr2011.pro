PRO ssoup_calprof, ll, band, photplam, ebvg, fprofs, fscalprof, ffcalprof, fscalprof0, ffcalprof0, $
  ...
  sprof    = make_array(nrtot,4)   ; surface brightnes four bands
  esprofs  = make_array(nrtot,4)   ; error in surface brightness from sky four bands
  esprofc  = make_array(nrtot)     ; Halpha continuum subtraction error surf bright
  esproft  = make_array(nrtot)     ; Halpha total error surf bright
  fprof    = make_array(nrtot,4)   ; flux four bands
  efprofs  = make_array(nrtot,4)   ; error in flux from sky 
  efprofc  = make_array(nrtot)     ; Halpha continuum subtraction error flux
  efproft  = make_array(nrtot)     ; Halpha total error surf bright
  ...
  ;
  ; derive continuum subtraction and total errors for Halpha
