PRO ssoup_multi_plothafuv, ll, sname, fsprof, fplot, title, dcorr=dcorr, kline=kline, $
                           stripbad=stripbad, noerr=noerr, connect=connect, highlight_last=highlight_last
  ;
  ; plot Halpha/FUV vs Halpha and R surface brightnesses
  ;
  ;  ll     -> logical unit of log file
  ;  sname  -> name of singg/hipass sources
  ;  fsprof -> name of calibrated surface brightness profile files
  ...
  ;            values.
  ;
  ; G. Meurer (ICRAR/UWA) 6/2010 original version ssoup_plothafuv
  ...
  odb       = 'singg_derived'
  udb1      = 'super_sungg_derived'
  udb2      = 'sungg_derived_2010'
  udb3      = 'sungg_derived_zz'
  prog      = 'SSOUP_MULTI_PLOTHAFUV: '
  ...
  xrange2   = [4.7, 10.0]                             ; R band surface brightness range
  aa        = angstsym()
  xmargin   = [8.,2.]
  ymargin   = [3.5,2.5]
  charsize  = 1.7
  symsize0  = 1.2                                     ; size of symbols previous obs
  symsize1  = 0.5                                     ; size of annuli
  ...
  plot, lshaon, lewfon, xrange=xrange1, yrange=yrange, xstyle=1, ystyle=1, psym=gsym(9), $
        xtitle=xtitleh, ytitle=ytitle, title=title, charsize=charsize, symsize=symsize0, $
        xmargin=xmargin, ymargin=ymargin, thick=thick, xthick=thick, ythick=thick, charthick=thick, $
        /nodata
  oplot, lshaon, lewfon, psym=gsym(1), symsize=symsize0, color=!gray
  oplot, lshaow, lewfow, psym=gsym(10), symsize=symsize0, color=!gray
  oplot, lshaou, lewfou, psym=gsym(10), symsize=0.8*symsize0, color=!gray
  ;
  ; plot weak or non detections (in Halpha) matches and highlight if found
  if npw gt 0 then begin 
     oplot, lshaow[pw], lewfow[pw], psym=gsym(10), symsize=symsize0, color=!dpink
  ENDIF ELSE BEGIN 
     IF npu GT 0 THEN BEGIN 
        oplot, lshaou[pu], lewfou[pu], psym=gsym(10), symsize=0.8*symsize0, color=!dpink
     ENDIF 
  ENDELSE 
  ;
  ; draw fiducial line
  ...
  ;
  ; loop through and overplot profiles
  FOR ii = 0, nf-1 DO BEGIN 
     ;
     ; adjust symsize if highlight_last is set and this is the last profile)
     test     = (keyword_set(highlight_last) AND float(ii EQ nf-1))
     symsize2 = (1.0+0.5*float(test))*symsize1
     IF test THEN clr = !purple ELSE clr = !black
     print, test, symsize2, clr
     ;
     ; read file
     plog,ll,prog,'reading in surface brightness profile file: '+fsprof[ii]
     readcol, fsprof[ii], sma, sr, esr, sha, eshat, eshas, eshac, snuv, esnuv, sfuv, esfuv, $
     ...
     if njhf gt 0 AND NOT keyword_set(stripbad) then begin 
        IF keyword_set(connect) THEN oplot, lsha[jhf], lewf[jhf], linestyle=1, thick=thick, color=clr   ; plottable points: dotted line linking
        oplot, lsha[jhf], lewf[jhf], symsize=symsize2, psym=gsym(9), color=clr       ; hollow points
     endif 
     if nkhf gt 0 then begin 
        ;
        ; points good in x & y: 
        ; solid line linking filled symbols with (optional) errors
        IF keyword_set(connect) THEN oplot, lsha[khf], lewf[khf], thick=thick, color=clr
        oplot, lsha[khf], lewf[khf], symsize=symsize2, psym=gsym(4), color=clr
        IF NOT keyword_set(noerr) THEN oploterror_old, lsha[khf], lewf[khf], elsha[khf], elewf[khf], $
                                       /nohat, errthick=thick+1, psym=3, errcolor=clr
     ENDIF
     IF NOT keyword_set(stripbad) THEN BEGIN 
        if nlhf gt 0 then oplot, lsha[lhf], lewf[lhf], symsize=symsize2, psym=gsym(19) ; lower limit in y 
        if nmhf gt 0 then oplot, lsha[mhf], lewf[mhf], symsize=symsize2, psym=gsym(18) ; upper limit in y 
        if nlh  gt 0 then oplot, lsha[lh], lewf[lh], symsize=symsize2, psym=gsym(20)   ; lower limit in x
        if nmh  gt 0 then oplot, lsha[mh], lewf[mh], symsize=symsize2, psym=gsym(21)   ; upper limit in x
     ENDIF 
  ENDFOR 
  ;
  ...
  plot, lsron, lewfon, xrange=xrange2, yrange=yrange, xstyle=1, ystyle=1, psym=gsym(9), $
        xtitle=xtitler, ytitle=ytitle, title=title, charsize=charsize, symsize=symsize, $
        xmargin=xmargin, ymargin=ymargin, thick=thick, xthick=thick, ythick=thick, charthick=thick, $
        /nodata
  oplot, lsron, lewfon, psym=gsym(1), symsize=symsize0, color=!gray
  oplot, lsrow, lewfow, psym=gsym(10), symsize=symsize0, color=!gray
  oplot, lsrou, lewfou, psym=gsym(10), symsize=0.8*symsize0, color=!gray
  ;
  ; plot weak or non detections (in Halpha) matches and highlight if found
  if npw gt 0 then begin 
     oplot, lsrow[pw], lewfow[pw], psym=gsym(10), symsize=symsize0, color=!dpink
  ENDIF ELSE BEGIN 
     IF npu GT 0 THEN BEGIN 
        oplot, lsrou[pu], lewfou[pu], psym=gsym(10), symsize=0.8*symsize0, color=!dpink
     ENDIF 
  ENDELSE 
  ;
  ; plot fiducial line
  ...
  ;IF nfound1 GT 0 AND (npw EQ 0 and npu eq 0) THEN oploterror_old, lsr1, lhafuv1, elsr1, elhafuv1, psym=gsym(1), symsize=symsize0, color=!dpink, errcolor=!dpink, /nohat
  ;IF nfound2 GT 0 THEN oploterror_old, lsr2, lhafuv2, elsr2, elhafuv2, psym=gsym(1), symsize=symsize0, color=!red, errcolor=!red, /nohat
  ;IF nfound3 GT 0 THEN oploterror_old, lsr3, lhafuv3, elsr3, elhafuv3, psym=gsym(1), symsize=symsize0, color=!green, errcolor=!green, /nohat
  ;
  ; loop through and overplot profiles
  FOR ii = 0, nf-1 DO BEGIN 
     ;
     ...
     ;
     ; plot profile and error
     if njhfr gt 0 AND NOT keyword_set(stripbad) then begin 
        IF keyword_set(connect) THEN oplot, lsr[jhfr], lewf[jhfr], linestyle=1, thick=thick, color=clr   ; plottable points: dotted line linking
        oplot, lsr[jhfr], lewf[jhfr], symsize=symsize2, psym=gsym(9), color=clr       ; hollow points
     endif 
     if nkhfr gt 0 then begin 
        ;
        ; points good in x & y: 
        ; solid line linking filled symbols with (optional) errors
        IF keyword_set(connect) THEN oplot, lsr[khfr], lewf[khfr], thick=thick, color=clr
        oplot, lsr[khfr], lewf[khfr], symsize=symsize2, psym=gsym(4), color=clr
        IF NOT keyword_set(noerr) THEN oploterror_old, lsr[khfr], lewf[khfr], elsha[khfr], elewf[khfr], $
                                       /nohat, errthick=thick+1, psym=3, errcolor=clr
     ENDIF
     IF NOT keyword_set(stripbad) THEN BEGIN 
        if nlhf gt 0 then oplot, lsr[lhf], lewf[lhf], symsize=symsize2, psym=gsym(19) ; lower limit in y 
        if nmhf gt 0 then oplot, lsr[mhf], lewf[mhf], symsize=symsize2, psym=gsym(18) ; upper limit in y 
        if nlh  gt 0 then oplot, lsr[lr], lewf[lr], symsize=symsize2, psym=gsym(20)   ; lower limit in x
        if nmh  gt 0 then oplot, lsr[mr], lewf[mr], symsize=symsize2, psym=gsym(21)   ; upper limit in x
     ENDIF 
  ENDFOR 
  ;;
  ;; plot profile and error
  ...
  ;   oplot, lsr[jhfr], lewf[jhfr], symsize=symsize1, psym=gsym(9), color=!black ; linking hollow points
  ;endif 
  ;if nkhf gt 0 then begin 
  ;   ;
  ;   ; points good in x & y: 
  ...
  ;   oplot, lsr[khfr], lewf[khfr], symsize=symsize1, psym=gsym(4), color=!black
  ;   oploterror_old, lsr[khfr], lewf[khfr], elsr[khfr], elewf[khfr], $
  ;               /nohat, errthick=thick+1, psym=3, errcolor=!black
  ;endif
  ;if nlhf gt 0 then oplot, lsr[lhf], lewf[lhf], symsize=symsize1, psym=gsym(19)  ; lower limit in y 
  ;if nmhf gt 0 then oplot, lsr[mhf], lewf[mhf], symsize=symsize1, psym=gsym(18)  ; upper limit in y 
  ;if nlh  gt 0 then oplot, lsr[lr], lewf[lr], symsize=symsize1, psym=gsym(20)    ; lower limit in x
  ;if nmr  gt 0 then oplot, lsr[mr], lewf[mr], symsize=symsize1, psym=gsym(21)    ; upper limit in x
  ;
  ; ------------------------------------------------------------------
  ; finish plot
