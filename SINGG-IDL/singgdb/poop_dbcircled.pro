function poop_dbcircled, ra_cen, dec_cen, radius, dis, sublist,SILENT=silent, $
...
; EXPLANATION:
;      Database must include items named 'RA' (in degrees) and 'DEC' (in degrees)
;      and must have previously been opened with DBOPEN
;
; CALLING SEQUENCE:
;     list = DBCIRCLED( ra_cen, dec_cen, [radius, dis, sublist, /SILENT, 
;                                          TO_B1950, /TO_J2000 ] )   
;
; INPUTS:
...
;
; OPTIONAL INPUT:
;       RADIUS - Radius of the search field in arcmin, scalar.
;               DBCIRCLED prompts for RADIUS if not supplied.
...
;       /SILENT - If this keyword is set, then DBCIRCLED will not print the 
...
;       IDL> list = dbcircled( ten(1,33,50.9), ten(3,39,36.7), 40)
;
; PROCEDURE CALLS:
...
;      make version for working with decimal degrees, G. Meurer
;-                   
 On_error,2

 if N_params() LT 2 then begin
    print,'Syntax - list = DBCIRCLED( ra, dec, radius, [ dis, sublist  '
 ...
 if (N_elements(ra_cen) NE 1) OR (N_elements(dec_cen) NE 1) then begin
    print, 'DBCIRCLED: ERROR - Expecting scalar RA and Dec parameters'
    if N_elements(sublist) GT 0 then return, sublist else return,lonarr(1)-1
 endif

 if N_params() LT 3 then read,'Enter search radius in arcmin: ',radius
 ...
 size = radius/60.      ;size = Size of search field in degrees
 decmin = double(deccen-size) > (-90.)
 decmax = double(deccen+size) < 90.
 bigdec = max(abs([decmin, decmax]))
 if abs(bigdec) EQ 90 then rasize = 360.0 else $             ;Updated Sep 1999
       rasize = abs(size/(cos(bigdec/!RADEG))) < 360.0  ;Correct for latitude effect

 if 2.0*rasize gt 360. then begin         ;Only need search on Dec?
...
;  If minimum RA is less than 0, or maximum RA is greater than 360
;  then we must break up into two searchs
 ...
 ENDELSE 
 print, 'number of elements in list = '+numstr(n_elements(list))
...
; Use GCIRC to compute angular distance of each source to the field center
 ...
 if max(list) GT 0 then begin                         ;Any entries found?
        print, 'entries found '+numstr(n_elements(list))
        dbext, list, 'RA,DEC', ra_match, dec_match
        gcircd,2, racen, deccen, ra_match, dec_match, ddis
 ...
 if not silent then $
       print,'No entries found by dbcircled in ', db_info( 'NAME',0 )
 return,lonarr(1)-1
