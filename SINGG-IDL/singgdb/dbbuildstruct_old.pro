pro dbbuildstruct,struct,NOINDEX=noindex,STATUS=status,SILENT=silent
;+
; NAME:
;	DBBUILDSTRUCT
; PURPOSE:
...
; REVISION HISTORY:
;
;       This is my old version, Dan Hanish seems to have a better
;       version so am using that instead.
;
...
;       Adapted from dbbuild to allow building of a database from
;         an arbitrary structure                        D. Hanish   Oct 2003
;-
...
;  will be updated below.
  ...
  for ii = 0l, Nv - 1 do begin
    ...
    FOR jj = 1,num_vars DO BEGIN
      s = SIZE(struct.(jj-1))
;;      IF s[0] EQ 0 or s[0] EQ 1 THEN BEGIN
      temp = struct.(jj-1)
      val = temp[i1[jj]:i2[jj]]
      dbxput,val,entry, idltype[jj],sbyte[jj],nbyte[jj]
;;      ENDIF ELSE BEGIN
;;        PRINT,"ERROR in dbbuildstruct: structure can't have more than 1 dimension"
;;        RETURN

;;        dbxput,struct.(jj-1)[i1[jj]:i2[jj]],entry, $
;;               idltype[jj],sbyte[jj],nbyte[jj]
;;      ENDELSE
    ENDFOR

    print, 'writing entry: ', ii
