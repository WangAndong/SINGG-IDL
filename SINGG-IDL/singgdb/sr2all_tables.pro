PRO sr2all_tables
  ...
  tab1    = 'sr2all_elg_opt.dat'  ; optical data for elgs
  tab2    = 'sr2all_elg_uv.dat'   ; uv-optical data for elgs
  tol     = 0.0001
  ...
  dbext, list0, 'runid,object,logmhi,distance,entry_sample,filter_r,filter_n', $
  ...
  ;
  ; Get ELG level data
  dbext, list0, 'name,optid,ra,dec,axerat,inclination,pa,a_r_gal,a_n_gal,frac_ha0_t,mabs_r0_t,err_mag_r_t', $
         sname,optid,ra,dec,arat,incl,pa,ar,an,fracha,mabsr,emabsr
  dbext, list0, 'rmax_f,radius_f,re_r_t,err_re_r_t,re_ha_t,err_re_ha_t,err_re_ha_t_cont,err_re_ha_t_sky', $
         rmax, radius, rer, erer, reha, ereha, erehac, erehas
  dbext, list0, 'r90_r_t,err_r90_r_t,r90_ha_t,err_r90_ha_t,err_r90_ha_t_cont,err_r90_ha_t_sky', $
         r90r, er90r, r90ha, er90ha, er90hac, er90has
  dbext, list0, 'logf_ha0_t,logl_ha0_t,err_logf_ha_t,err_logf_ha_t_cont,err_logf_ha_t_sky', $
         lfha0, lha0, elha0, elha0c, elha0s
  dbext, list0, 'logse_ha0_t,err_logse_ha_t,err_logse_ha_t_cont,err_logse_ha_t_sky', $
         lseha, elseha, elsehac, elsehas 
  dbext, list0, 'ew50_0_t,err_ew50_0_t,mu_e_r0_t,err_mu_e_r_t', $
  ...
  fmt1   = '(a28,f11.5,f11.5,f6.2,f7.1,f7.1,f7.3,f7.3,f7.3,f9.3,f9.3,f9.3,f9.3,f8.1,f7.2,f9.2,f8.2,f9.2,f8.2,f9.2,f8.2,f9.2,f8.2,f9.3,f9.3,f8.1,f7.1)'
  ;
  ...
  dbext,listn,'entry_othfilt',listf ; corresponding fuv entries
  ;
  ; match by entry in singg_derived
  entf  = dbmatch('entry_singg_derivd', list0, listf)   ; fuv matches
  entn  = dbmatch('entry_singg_derivd', list0, listn)   ; nuv matches
  ;
  ...
  IF nk1 GT 0 THEN BEGIN 
     print, '#entf   entfn'
     forprint, entf[k1], entfn[k1],'  '+snamef[k1]+'   '+snamen[k1]
     keywait,'type anything to continue - '
  ENDIF 
  print, 'number of nuv entry_othfilt mismatches: ', nk2
  IF nk2 GT 0 THEN BEGIN 
     print, '#entn   entnf'
     forprint, entn[k2], entnf[k2],'  '+snamef[k2]+'   '+snamen[k2]
     keywait,'type anything to continue - '
  ENDIF 
  ;
  ...
  IF nkk GT 0 THEN BEGIN
     ...
     forprint, sname[kk]+'  '+snamef[kk]+'  '+snamen[kk]+'  ', rat_rmax[kk], rat_area[kk], rat_arat[kk]
     print,'******'
     forprint, sname[kk]+'  '+snamef[kk]+'  '+snamen[kk]+'  ', dif_ra[kk], dif_dec[kk], dif_pa[kk]
     keywait,'type anything to continue - '
  ENDIF 
  ;
