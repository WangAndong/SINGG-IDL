pro grm_imdbase,hdr, catalogue, list, XPOS=xpos, YPOS=ypos, $
...
;               are considered.
;     RADEG   - if set then the database RA entries are in degrees
;               rather than hours.
;
; NOTES:
...
;      Add RADEG keyword for databases where RA is in degrees. G. Meurer
;          Feb. 2005
;-
...
; Compare equinox of image with that of database and precess if necessary
 ...
 if (ramax - ramin) GT 0.5*rabound then begin    ;Does the RA cross 24 hours?
     newmax = ramin
     ramin = ramax
     ramax = rabound
     redo = 1
...
;stop
 ...
 if nstar GT 0 then begin                     ;Any stars found?
   ...
   IF NOT keyword_set(radeg) THEN ra = ra*15. 
