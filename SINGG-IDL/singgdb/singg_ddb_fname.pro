FUNCTION singg_ddb_fname, entry, option, hpath=hpath, nopath=nopath
   ...
   ; option  -> string describing which file name to return.  The 
   ;            following table gives an index (described below) 
   ;            and valid values for option:
   ...
   ; procedure to work
   ;
   ; G. Meurer (JHU) 07/2007
   ; G. Meurer (ICRAR/UWA) 09/2011 
   ;    * Adjust to take paths relative to $SHOME environment variable
   ;
   ...
   ;
   ; get $SHOME (SINGG Home) environment variable
   shome   = getenv('SHOME')
   if strmid(shome,0,1,/reverse_offset) ne '/' then shome = shome+'/'
   ; ****
   ;
   ; set various paths
   hp      = shome+'HSINGG/'
   IF keyword_set(hpath) THEN pathi1 = hpath ELSE pathi1 = hp
   patho1  = shome+'SR2QA/Plots/'
   patho2  = shome+'SR2QA/Images/'
   patho3  = shome
   patho4  = shome+'SR2QA/Data/'
   ;
   ; **** old path defaults maybe make an /old switch?
