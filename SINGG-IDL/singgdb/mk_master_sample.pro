PRO mk_master_sample, ingest=ingest
   ...
   ;    add new found galaxies
   ; G. Meurer (ICRAR/UWA) 4/2012
   ;    change telescope code in SELOPTRC
   ;    put galaxies observed for SIGRID and CHOIRS in to STATOPTRC
   ; G. Meurer (ICRAR/UWA) 6/2012
   ;    checks for multiple entries with the same SNAME, and add 
   ;    precautions to stop this from happening with the "newly
   ;    found" galaxies.
   ;
   ; start up logging
   ...
   wd        = shome+'Sample/'                     ; directory to start work in
   ;updatestr = 'Date: 2010 Apr 7.'                
   ;filk      = 'sample_kin_stat1004.dat'
   ;updatestr = 'Date: 2010 Oct 4'                 
   ;filk       = 'sample_kin_stat1010.dat'
   ;updatestr  = 'Date: 2011, Nov 25'              
   ;filk       = 'sample_kin_stat1110.dat'
   ;updatestr  = 'Date: 2012, Apr 24'              
   ;updatestr  = 'Date: 2012, Jun 21'               ; **** make sure to update for each new ingest!!!
   ;filk       = 'sample_kin_stat1204.dat'
   updatestr  = 'Date: 2012, Nov 1'                 ; **** make sure to update for each new ingest!!!
   filk       = 'sample_kin_stat1211.dat'
   sdir       = shome+'Sample/'
   ;fsigrid   = sdir+'sigrid_singg.txt'
   fsigrid    = sdir+'SIGRiD_f8shortA.dat'         ; Galaxies in SIGRID
   flvhis     = sdir+'lvhis.dat'                   ; Galaxies in LVHIS survey
   fmgoose    = sdir+'gmd.txt'                     ; Mhongoose precursor sample
   fs4g       = sdir+'s4g_table1.dat'              ; S4G survey 
   ;fnewf      = sdir+'sr2_newfound.dat'            ; SINGG new found galaxies (in SR2 data but not yet singg_derived db)
   fnewf      = sdir+'sr2_newfound_fixed.dat'      ; SINGG new found galaxies (in SR2 data but not yet singg_derived db)
   fnodetect  = sdir+'nondetect.dat'
   fchoirs    = sdir+'choirs_observed.dat'         ; galaxies observed on ANU2.3m for CHOIRS project
   rmatch     = 4.0  ; arcmin
   minsradfir = 1.0  ; arcmin
   ...
   dbf       = 'super_sungg_derived'     ; SUNGG measurement database
   ...
   dbout     = 'supersingg_master'       ; output database
   ...
   item     = ['sname', 'hname', 'ra', 'dec', 'glong', 'glat', 'possrc', 'ebv', 'dist', 'lmhi',$
               'obs', 'fhadetect', 'lf09akari', 'lf90akari', 'lf90ha', 'lf90uv', $
               'sradfir', 'nakirc', 'nakfis', 'niras', 'densakirc', 'densakfis', 'sepakirc', 'sepakfis', 'sepiras', $
   ...
   tout     = ['C', 'C', 'R', 'R', 'R', 'R', 'I', 'R', 'R', 'R', 'C', 'I', 'R', 'R', 'R', 'R', $
               'R', 'I', 'I', 'I', 'I', 'I', 'R', 'R', 'R', 'I', 'I', 'B', 'B', 'I', 'C', 'I', $
               'I', 'B', 'B', 'B', 'I', 'I', 'I', 'I', 'I', 'I', 'I', 'I', 'I', 'I']
   tlen     = [0, 0, 8, 8, 8, 8, 2, 4, 4, 4, 0, 2, 4, 4, 4, 4, 4, 2, 2, 2, 4, 4, $
   ...
   descript = ['SINGG name', 'HIPASS name', 'RA (J2000) [deg]', 'Dec (J2000) [deg]', $
               ...
               'Halpha detect flag [0:no,1:yes,-1:not obsvd]', $
               ...
               'Search radius for IRAS & AKARI matches [arcsec]', 'Number of AKARI IRC matches', $
               ...
               'Entry in '+strupcase(dbs)+' db', 'Entry in '+strupcase(dbu)+' db', $
               'Entry in '+strupcase(dbh)+' db', 'FUV entry in '+strupcase(dbf)+' db', 'NUV entry in '+strupcase(dbf)+' db', $
               'Entry in '+strupcase(dbir)+' db', 'Closest entry in GALEX db ('+strupcase(dbg)+')', 'Deepest entry in GALEX db ('+strupcase(dbg)+')', $
               'Entry in AKARI IRC db ('+strupcase(dbai)+')', 'Entry in AKARI FIS database ('+strupcase(dbaf)+')']
   ...
   hname1    = strtrim(hname1,2)
   sname0    = strtrim(sname0,2)
   dbclose
   plog, llog, logstr, 'Number of galaxies in SINGG_derived: '+numstr(nsr2)
   ;
   ; ~~~~~~~~~~~~~~~~ add Halpha newly found galaxies ~~~~~~~~~~~~~~~~~
   plog, llog, logstr, 'Reading file of "newly found" Halpha sources: '+fnewf
   fmtn      = '(a,i,i,a,a,a,a,a,a)'
   readcol, fnewf, snamenf, xpixnf, ypixnf, rastrnf, decstrnf, runidnf, filtnbnf, fnetn, format=fmtn
   nnf       = n_elements(snamenf)
   ;
   ; check names compared to what is in singg_derived, 
   keepnf     = make_array(nnf, /byte, value=1b)
   for jj = 0, nnf-1 do begin 
      pp     = strpos(sname0, snamenf[jj])
      kk     = where(pp ge 0, nkk)
      if nkk gt 0 then begin
         plog, llog, logstr, 'The following "newly found" galaxy already is in '+dbh+' : '+snamenf[jj]+' . Will use DB version...'
         keepnf[jj] = 0b
      endif
   endfor 
   ;
   ; and do not proceed with those that are in singg_derived
   kk        = where(keepnf gt 0b, nnf)
   if nnf gt 0 then begin 
      ;
      ; save data on the sources to keep
      snamenf   = snamenf[kk]
      xpixnf    = xpixnf[kk]
      ypixnf    = ypixnf[kk]
      rastrnf   = rastrnf[kk]
      decstrnf  = decstrnf[kk]
      runidnf   = runidnf[kk]
      filtnbnf  = filtnbnf[kk]
      fnetn     = fnetn[kk]
      ;
      ; convert positions to decimal degrees
      ranf      = 15.0*sexideg(rastrnf)
      decnf     = sexideg(decstrnf)
      ;
      ; derive HIPASS name from snamenf, and match to other SR2 galaxies
      ; so as to steal the entry in the sample database
      ii        = strpos(snamenf,':')
      bad       = where(ii LT 0, nbad)
      IF nbad GT 0 THEN stop, logstr+"**** can't parse galaxy names in "+fnewf
      hnamenf   = make_array(nnf,/string,value='')
      esampnf   = make_array(nnf,/long,value=-1l)
      FOR jj = 0, nnf-1 DO BEGIN 
         hnamenf[jj] = strmid(snamenf[jj],0,ii[jj])
         pp          = where(hname1 EQ hnamenf[jj], npp)
         IF npp GT 0 THEN esampnf[jj] = esamp[pp[0]]
      ENDFOR 
      ;
      ; fill in dummy values for some arrays mabsr,lfha,ropt,esamp,eiras,lf90ha0
      mabsrnf   = make_array(nnf, /float, value=lfflag)
      lfhanf    = make_array(nnf, /float, value=lfflag)
      roptnf    = make_array(nnf, /float, value=lfflag)
      eirasnf   = make_array(nnf, /long, value=-1l)
      enthnf    = make_array(nnf, /long, value=-10l)
      ;
      ; append the newly found galaxies
      plog, llog, logstr, 'appending data on '+numstr(nnf)+' newly found galaxies to arrays'
      enth      = [enth, enthnf]
      sname0    = [sname0, snamenf]
      hname1    = [hname1, hnamenf]
      mabsr     = [mabsr, mabsrnf]
      lfha      = [lfha, lfhanf]
      ropt      = [ropt, roptnf]
      esamp     = [esamp, esampnf]
      ...
      plog, llog, logstr, '**** warning no newly found galaxies to append '
   endelse 
   ;
   ; ~~~~~~ mark blank fields ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   plog, llog, logstr, 'marking blank fields...'
   fhadetect = make_array(nsr2p, /int, value=1)    ; starting with SR2 so default is they are detected
   readcol, fnodetect, nambl, format='(a)'
   nbl   = n_elements(nambl)
   nambl = strtrim(nambl,2)
   FOR ii = 0, nbl-1 DO BEGIN 
      jj = strpos(hname1, nambl[ii])
      kk = where(jj GE 0, nkk)
      IF nkk GT 0 THEN BEGIN 
         plog, llog, logstr, 'Marking field as blank: '+nambl[ii]+' number of matches = '+numstr(nkk)
         fhadetect[kk] = 0
      ENDIF 
   ENDFOR 
   ;
   ; calculate estimated akari 90 micron flux ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   plog, llog, logstr, 'estimating 90 micron flux from R and Halpha fluxes'
   lf90ha0   = lcha + lfha - 0.137*mabsr
   qq        = where(lfha LE lfflag OR mabsr LE lfflag, nqq)
   ...
   ; * ptr01: for galaxies in sample marks position of first SR2 source
   ; * ptr10: for galaxies in SR2 marks position in sample
   plog, llog, logstr, 'setting up pointers'
   kk        = where(sname0 eq hname1 or strpos(sname0,'S1') ge 7, nkk)
   jj        = sort(esamp[kk])
   uu        = uniq(esamp[kk[jj]])
   nuu       = n_elements(uu)
   pp        = kk[jj[uu]]
   ...
   ptr10     = make_array(nsr2p, /long, value=-1l)
   ;
   ; set some pointers
   ...
   ra               = [ra1, ra0[qq]]
   dec              = [dec1, dec0[qq]]
   possrc           = [make_array(nsr2p, /int, value=1), make_array(nstilltodo, /int, value=0)]
   fhadetect        = [fhadetect, make_array(nstilltodo, /int, value=-1)]
   ...
   IF nkk GT 0 THEN haflag[kk] =  1
   ;
   ; For Halpha nondetections change RA, DEC to HI coords, 
   ; and possrc to 0
   plog, llog, logstr, 'reverting coords to HI positions for Ha nondetections'
   jj             = where(fhadetect EQ 0, njj)
   IF njj GT 0 THEN BEGIN 
      plog, llog, logstr, 'will revert to HI coords for '+numstr(njj)+' HIPASS sources'
      ra[jj]      = ra0[ptr10[jj]]
      dec[jj]     = dec0[ptr10[jj]]
      possrc[jj]  = 0
      str         = 'sources with reverted coords: '
      FOR ii = 0, njj-1 DO str = str + hname[jj[ii]]+' '
      plog, llog, logstr, str
   ENDIF ELSE BEGIN 
      plog, llog, logstr, 'no galaxies need to have their coords reverted.  ?? huh??? '
   ENDELSE 
   ;
   ...
   plog, llog, logstr, 'opening GALEX tile database: '+dbg+' and cross matching'
   dbopen, dbg
   list        = dbfind('ow = 0')
   ;
   ...
   IF n9 GT 0 THEN BEGIN 
      ...
      jj          = where(entoc GT 0 AND (raoc GE 0.0 AND decoc GT -90.0), njj)
      IF njj GT 0 THEN BEGIN 
   ...
   FOR ii = 0, ntile-1 DO BEGIN 
      ...
      IF list5[0] GE 0 THEN BEGIN 
         ...
         FOR jj = 0, nl5-1 DO BEGIN
            ...
            pp  = where(strpos(strupcase(hname),str) EQ 0, npp)
   ...
   IF nockeep GT 0 THEN BEGIN 
      ...
      IF nockeep GT 0 THEN BEGIN 
         ...
         fhadetect        = [fhadetect, make_array(nockeep, /int, value=-1)]
         ...
         sradfir          = [sradfir, make_array(nockeep, /float, value=-1.0)]
   ...
   ;
   ; make sure sname doesn't have extra fluff
   sname       = temporary(strtrim(sname,2))
   ;
   ; ~~~~~~~~~ redo IRAS matching ~~~~~~~~~~~~~~~~~~~~~~~
   ;
   ...
   FOR ii = 0, nmast-1 DO BEGIN 
      srad       = sradfir[ii]     ; search radius in arcsec
      ;
   ...
   IF njj GT 0 THEN scode[jj] = 5
   jj            = where(telk EQ 'SALT',njj)
   IF njj GT 0 THEN scode[jj] = 6
   ; match by singg_sample entry, check object name
   FOR kk = 0, nkin-1 DO BEGIN 
      jj = where(entsamp EQ esink[kk], njj)
      IF njj GT 0 THEN BEGIN 
         qq = where(strtrim(strupcase(sname[jj]),2) EQ strtrim(strupcase(snamek[kk]),2), nqq)
         CASE nqq OF 
   ...
   ; LVHIS, SIGRID, and MHONGOOSE, and whether the
   ; galaxy has been observed spectroscopically for CHOIRS or SIGRID
   ...
   lchobs     = make_array(nmast, /byte, value=0b) 
   ;
   ...
   ; ------------------------------------------------------------------
   ; Read in list of CHOIRS galaxies that have already been observed
   plog, llog, logstr, 'Reading list of galaxies that have been observed for CHOIRS'
   readcol, fchoirs, chgal, comment='#', format='(a)'
   nchgal = n_elements(chgal)
   plog, llog, logstr, 'Read '+numstr(nchgal)+' names from: '+fchoirs
   nchmtch = 0
   for ii = 0, nchgal-1 do begin 
      ; qq = where(strpos(strupcase(sname),strupcase(chgal[ii])) ge 0, nqq)
      str     = strtrim(chgal[ii],2)
      qq      = where(strmatch(sname, str, /fold_case) ge 1, nqq)
      if nqq ge 1 then begin 
         lchobs[qq[0]] = 1b 
         nchmtch       = nchmtch + 1
      endif 
      if nqq gt 1 then plog, llog, logstr, 'WARNING! CHOIRS observed source '+chgal[ii]+' matched multiple times ???'
      IF nqq EQ 0 THEN plog, llog, logstr, 'WARNING! galaxy '+chgal[ii]+' not matched to a SNAME'
   endfor 
   plog, llog, logstr, 'Number of CHOIRS galaxies matched: '+numstr(nchmtch)
   ;
   ; set reported rotcurve status to SG for those marked
   ; Observed for SIGRID.
   qq      = where(lsigobs ge 1 and strtrim(statoptrc,2) ne '' and strpos(strtrim(statoptrc,2),'SG') lt 0, nqq)
   if nqq gt 0 then statoptrc[qq] = 'SG,'+statoptrc[qq]
   qq      = where(lsigobs ge 1 and strtrim(statoptrc,2) eq '', nqq)
   if nqq gt 0 then statoptrc[qq] = 'SG.|'
   ;
   ; set reported rotcurve status to CH for those marked
   ; Observed for CHOIRS.
   qq      = where(lchobs ge 1 and strtrim(statoptrc,2) ne '' and strpos(strtrim(statoptrc,2),'CH') lt 0, nqq)
   if nqq gt 0 then statoptrc[qq] = 'CH,'+statoptrc[qq]
   qq      = where(lchobs ge 1 and strtrim(statoptrc,2) eq '', nqq)
   if nqq gt 0 then statoptrc[qq] = 'CH.|'
   ;
   ; -------------------------------------------
   ;
   ; check some entries that should be unique
   ptr         = indgen(nmast)
   jj          = notuniq(sname)
   njj         = n_elements(jj)
   IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
   IF njj GT 1 THEN BEGIN
      plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated values in SNAME'
      FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr[jj[ii]])+'  SNAME = '+sname[jj[ii]]
   ENDIF 
   ;
   kk          = where(entoptphot GE 1, nkk)
   IF njj GT 0 THEN BEGIN 
      ent2     = entoptphot[kk]
      ptr2     = ptr[kk]
      sn2      = sname[kk]
      ...
      IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
      IF njj GT 1 THEN BEGIN
         plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated valid values in ENTOPTPHOT'
         FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr2[jj[ii]])+'  ENTOPTPHOT = '+ent2[jj[ii]]+'  SNAME = '+sn2[jj[ii]]
      ENDIF 
   ENDIF 
   ;
   kk          = where(entfuvphot GE 1, nkk)
   IF njj GT 0 THEN BEGIN 
      ent2     = entfuvphot[kk]
      ptr2     = ptr[kk]
      sn2      = sname[kk]
      ...
      IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
      IF njj GT 1 THEN BEGIN
         plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated valid values in ENTFUVPHOT'
         FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr2[jj[ii]])+'  ENTFUVPHOT = '+ent2[jj[ii]]+'  SNAME = '+sn2[jj[ii]]
      ENDIF 
   ENDIF 
   ;
   kk          = where(entnuvphot GE 1, nkk)
   IF njj GT 0 THEN BEGIN 
      ent2     = entnuvphot[kk]
      ptr2     = ptr[kk]
      sn2      = sname[kk]
      ...
      IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
      IF njj GT 1 THEN BEGIN
         plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated valid values in ENTNUVPHOT'
         FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr2[jj[ii]])+'  ENTNUVPHOT = '+ent2[jj[ii]]+'  SNAME = '+sn2[jj[ii]]
      ENDIF 
   ENDIF 
   ;
   kk          = where(entiras GE 1, nkk)
   IF njj GT 0 THEN BEGIN 
      ent2     = entiras[kk]
      ptr2     = ptr[kk]
      sn2      = sname[kk]
      ...
      IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
      IF njj GT 1 THEN BEGIN
         plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated valid values in ENTIRAS'
         FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr2[jj[ii]])+'  ENTIRAS = '+ent2[jj[ii]]+'  SNAME = '+sn2[jj[ii]]
      ENDIF 
   ENDIF 
   kk          = where(entakirc GE 1, nkk)
   IF njj GT 0 THEN BEGIN 
      ent2     = entakirc[kk]
      ptr2     = ptr[kk]
      sn2      = sname[kk]
      ...
      IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
      IF njj GT 1 THEN BEGIN
         plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated valid values in ENTAKIRC'
         FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr2[jj[ii]])+'  ENTAKIRC = '+ent2[jj[ii]]+'  SNAME = '+sn2[jj[ii]]
      ENDIF 
   ENDIF 
   ;
   kk          = where(entakfis GE 1, nkk)
   IF njj GT 0 THEN BEGIN 
      ent2     = entakfis[kk]
      ptr2     = ptr[kk]
      sn2      = sname[kk]
      ...
      IF njj EQ 1 AND jj[0] EQ -1 THEN njj = 0
      IF njj GT 1 THEN BEGIN
         plog, llog, logstr, 'WARNING: there are '+numstr(njj)+' repeated valid values in ENTAKFIS'
         FOR ii = 0, njj-1 DO plog, llog, logstr, '   pre-sort entry: '+numstr(ptr2[jj[ii]])+'  ENTAKFIS = '+ent2[jj[ii]]+'  SNAME = '+sn2[jj[ii]]
      ENDIF 
   ENDIF 
   ;
   ; -------------------------------------------
   ...
   fhadetect   = temporary(fhadetect[kk])
   ...
   dbopen, dbout, 1
   dbbuild_new, sname, hname, ra, dec, glong, glat, possrc, ebv, dist, lmhi, obs, fhadetect, $
                lf09irc, lf90fis, lf90ha, lf90uv, sradfir, nentirc, nentfis, nmiras, ndensirc, ndensfis, $
